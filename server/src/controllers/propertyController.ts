...
export const getProperties = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (city) filters.city = { equals: String(city), mode: 'insensitive' };
    if (type) filters.type = String(type);
    if (purpose) filters.purpose = String(purpose);
    ...
    if (bedrooms) filters.bedrooms = parseInt(String(bedrooms));
    if (bathrooms) filters.bathrooms = parseInt(String(bathrooms));

    if (minAreaSize || maxAreaSize) {
      filters.areaSize = {};
      if (minAreaSize) filters.areaSize.gte = parseFloat(String(minAreaSize));
      if (maxAreaSize) filters.areaSize.lte = parseFloat(String(maxAreaSize));
    }
  ...
  try {
    ...
    if (images && Array.isArray(images)) {
      ...
      await prisma.propertyImage.createMany({
        data: images.map((url: string) => ({ propertyId: id, url })),
      });
    }
...
export const getRecommendations = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Default Fallbacks
    if (favoriteCities.length === 0) favoriteCities = ['Islamabad', 'Lahore', 'Karachi'];
    if (favoriteTypes.length === 0) favoriteTypes = ['HOUSE', 'APARTMENT'];
